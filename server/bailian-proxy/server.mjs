...
export function createHandler(config, { fetchImpl = fetch, rateStore = new Map() } = {}) {
  return async function handler(request, response) {
    ...
    try {
      ...
      return json(response, status, { error: status === 429 ? "UPSTREAM_RATE_LIMITED" : "UPSTREAM_UNAVAILABLE" }, allowedOrigin);
    }
  };
}
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  createServer(createHandler(config)).listen(port, "0.0.0.0", () => {
    console.info(`十一说 Bailian proxy listening on ${port}; model=${CHAT_MODEL}; voice=local-kokoro-client`);
  });
}
